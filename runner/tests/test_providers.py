"""Local provider store: resolution (which env to inject for a run's llm params),
storage (0600 toml), and the `adb-runner providers` CLI."""
...
from adb_runner import providers


@pytest.fixture
def cfg(tmp_path, monkeypatch):
    path = tmp_path / "providers.toml"
    monkeypatch.setenv("ADB_PROVIDERS_FILE", str(path))
    return path


def test_save_load_roundtrip_is_0600(cfg):
    providers.save({"openai": {"OPENAI_API_KEY": "sk-secret", "OPENAI_BASE_URL": "http://h/v1"}})
    assert providers.load() == {
...
def test_env_for_run_resolves_bare_llm(cfg):
    providers.save({"openai": {"OPENAI_BASE_URL": "http://llama/v1", "OPENAI_API_KEY": "k"}})
    manifest = _manifest({"kind": "llm"})
    env = providers.env_for_run(manifest, {"model": "openai/qwen3.5-9b"})
...
def test_mock_and_unconfigured_yield_nothing(cfg):
    ...
    manifest = _manifest({"kind": "llm"})
    assert providers.env_for_run(manifest, {"model": "mock/model"}) == {}
    # a provider with no stored config contributes nothing
    assert providers.env_for_run(manifest, {"model": "anthropic/claude"}) == {}


def test_resolves_llm_inside_list_of_struct(cfg):
    # werewolf shape: players = listOf (struct { model = llm; role = enum; })
    providers.save({"anthropic": {"ANTHROPIC_API_KEY": "a"}, "openai": {"OPENAI_API_KEY": "o"}})
    manifest = {
        "name": "werewolf",
        "params": {
            "players": {
    ...
    env = providers.env_for_run(manifest, realized)
...
def test_resolves_param_wrapped_struct_field(cfg):
    ...
    realized = {"agents": [{"name": "A", "model": "openai/qwen3.5-9b"},
                           {"name": "B", "model": ""}]}
    assert providers.env_for_run(manifest, realized) == {"OPENAI_API_KEY": "k"}
...
def test_openai_api_service_ids_route_to_named_section(cfg):
    ...
    providers.save({"llama": {"LLAMA_API_KEY": "k", "LLAMA_BASE_URL": "http://l/v1"},
                    "openai": {"OPENAI_API_KEY": "real"}})
    manifest = _manifest({"kind": "llm"})
    env = providers.env_for_run(manifest, {"model": "openai-api/llama/qwen3.5-9b"})
    assert env == {"LLAMA_API_KEY": "k", "LLAMA_BASE_URL": "http://l/v1"}
    # a named service nobody configured is never gated (it may need nothing)
    assert providers.missing_providers(manifest, {"model": "openai-api/other/m"}) == []
    assert providers.section_for("openai-api/llama/qwen") == "llama"
    assert providers.section_for("openai/gpt-4o") == "openai"
    assert providers.section_for("mockllm/model") is None


def test_missing_providers_gate(cfg, monkeypatch):
    manifest = _manifest({"kind": "llm"})
    for var in ("OPENAI_API_KEY", "OPENAI_BASE_URL"):
        monkeypatch.delenv(var, raising=False)
    # known provider, nothing configured anywhere → reported
    assert providers.missing_providers(manifest, {"model": "openai/q"}) == ["openai"]
    # mocks (both spellings) reference no provider
    assert providers.missing_providers(manifest, {"model": "mock/model"}) == []
...
def _cli(argv, stdin="", monkeypatch=None):
    ...
    return providers.providers_cli(argv)
...
def test_cli_set_unknown_provider_prompts_conventional_names(cfg, monkeypatch, capsys):
    code = _cli(["set", "myprov"], stdin="k-123\nhttps://my.host/v1\n", monkeypatch=monkeypatch)
    assert code == 0
    assert providers.load()["myprov"] == {
