...
Storage: ``~/.config/adb/providers.toml`` (0600), one section per provider mapping env var
→ value. Managed with ``adb-runner providers set <provider>``, which prompts for every
...
def config_path() -> Path:
    ...
    return Path(base) / "adb" / "providers.toml"
...
def save(data: dict[str, dict[str, str]]) -> Path:
    ...
    lines = [
        "# ADB provider credentials & endpoints — managed by `adb-runner providers`.",
...
def section_for(model_id: str) -> str | None:
    ...
    *service* segment, so a named credential set (e.g. `providers set llama`) serves
...
def _cmd_list() -> int:
    store = load()
    if not store:
        print(f"no providers configured ({config_path()})")
        return 0
...
def _cmd_set(argv: list[str]) -> int:
    ...
    if len(argv) != 1:
        print("usage: adb-runner providers set <provider>\n"
...
def _cmd_remove(argv: list[str]) -> int:
    if not argv:
        print("usage: adb-runner providers remove <provider>", file=sys.stderr)
        return 2
    store = load()
...
def providers_cli(argv: list[str]) -> int:
    """`adb-runner providers <list|set|remove|path>` — manage local model credentials."""
    if not argv or argv[0] in ("-h", "--help"):
        print(
            "adb-runner providers — local model credentials & endpoints\n\n"
    ...
    if cmd == "path":
        print(config_path())
        return 0
    print(f"unknown providers command {cmd!r}", file=sys.stderr)
    return 2
