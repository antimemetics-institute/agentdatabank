...
# `fork` copies a registry experiment out into one, `bump` moves a repo's adb pin
# (nix side and uv side together), `pin` prints the rev.
#
...
# and asks for --rev instead of guessing. The experiments tree of the same adb is
# baked in too (what `fork` copies from) — content and pin come from one source.
{ lib, adb, origin, python313, writeShellApplication, git, uv, rev ? null }:
let
...
writeShellApplication {
  ...
  text = ''
    ...
    export ADB_EXPERIMENTS_DIR=${adb.cleanImport "adb-dev-experiments" ../../experiments}
