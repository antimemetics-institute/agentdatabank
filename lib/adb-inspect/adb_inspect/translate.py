...
def emit_model_event(ev: ModelEvent, agent: str,
    ...
    request = LLMRequest(
        messages=[m.model_dump(mode="json", exclude_none=True) for m in ev.input],
        params=_dump(ev.config) or {},
        raw=ev.call.request if ev.call is not None else None,
    )
    response = None
    if out.choices:
        response = LLMResponse(
            message=out.message.model_dump(mode="json", exclude_none=True),
            finish_reason=out.stop_reason,
            model=out.model,
            raw=ev.call.response if ev.call is not None else None,
        )
    latency = round(ev.working_time * 1000) if ev.working_time is not None else None
    error = LLMError(kind="model_error", message=str(ev.error)) if ev.error else None
    emit(
        LLMCall(
            agent=agent,
            model=ev.model,
            ...
            latency_ms=latency,
            error=error,
            meta={"instance_id": sample_id, "repeat": epoch},
        )
    )
    ...
    for m in turns:
        ...
        emit(
            Message(
                ...
                content=m.text or "",
                channel=channel,
                meta={"role": m.role, "instance_id": sample_id, "repeat": epoch},
            )
        )
    ...
    if results:
        for score in results.scores:
            for name, m in score.metrics.items():
                num = _num(m.value)
                if num is not None:
                    emit(Metric(name=f"{score.name}/{name}", value=num))

    ti = to = 0
    for usage in (log.stats.model_usage or {}).values():
        ti += usage.input_tokens or 0
        to += usage.output_tokens or 0
    emit(Metric(name="tokens_input", value=ti))
    emit(Metric(name="tokens_output", value=to))
